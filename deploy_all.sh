...
# 自动检测是否为 AutoDL 容器环境
if hostname | grep -q "autodl-container" || [ -d "/root/autodl-tmp" ]; then
    echo "  检测到 AutoDL 环境，自动执行网络配置..."
    echo ""
    echo "══════════════════════════════════════════════════"
    echo "  Phase 6: AutoDL 网络配置 (Nginx + 端口 6006)"
    echo "══════════════════════════════════════════════════"
    bash "$SCRIPT_DIR/scripts/05_setup_autodl.sh"
else
    echo "  普通服务器环境，跳过 AutoDL 配置"
    echo "  如需配置 Nginx 代理，可手动运行:"
    echo "    sudo bash $SCRIPT_DIR/scripts/setup_nginx.sh"
fi

SERVER_IP=$(curl -s --max-time 5 ifconfig.me 2>/dev/null || hostname -I | awk '{print $1}')
source "$SCRIPT_DIR/configs/env.conf" 2>/dev/null || true
ACCESS_PORT="${PUBLIC_PORT:-${DIFY_WEB_PORT:-3000}}"
...
echo "║                                                  ║"
echo "║   请在浏览器中访问:                              ║"
echo "║   http://${SERVER_IP}:${ACCESS_PORT}             ║"
echo "║                                                  ║"
echo "╚══════════════════════════════════════════════════╝"
